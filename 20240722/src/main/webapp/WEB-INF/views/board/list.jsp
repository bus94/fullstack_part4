...
<!-- 만약 아무것도 없을 경우에는 checked를 title로 설정하기 위해 if문 작성 -->
...
<section id="content">
	...
	<div id="board-list-container">
			...
			<c:if test="${not empty list}">
				<c:forEach var="board" items="${list}">
					<tr>
						...
						<td><a href="${path}/board/view?no=${board.no}"> <c:out
									value="${board.title}" />
						</a></td>
						<td><c:out value="${board.writerId}" /></td>
						<td><fmt:formatDate type="date" value="${board.createDate}" />
						</td>
		...
		<div align="center">
			<!--이전 페이지 -->
			<button
				onclick="movePage('${path}/board/list?page=${pageInfo.prevPage}');">&lt;</button>

			<%-- 10개 페이지 
 					EL 태그 ${}
 					- getter 메서드를 통해서 속성 값을 가져오도록 설계가 되어있다. (private 변수를 사용해도 자동으로 가져옴)
 
 					pageInfo.startPage
 					getStartPage() 메서드를 호출
 
 					만약 클래스 변수가 없을 경우에는 메서드명을 찾아서 메서드명이 일치하면 메서드를 실행해서 결과를 받아온다.
 			--%>
			...
			<!--다음 페이지 -->
			<button
				onclick="movePage('${path}/board/list?page=${pageInfo.nextPage}');">&gt;</button>
		</div>
		<!-- page부 종료 -->
	</div>
